...
if __name__ == '__main__':
	...
	d_modulo_to_l_t_factors_t_cycle_all = {}
	d_modulo_to_arr_n = {}
	...
	for modulo in range(1, max_modulo + 1):
		s_cycle_all = set()
		l_t_factors_t_cycle_all = []
		d_len_cycle_to_s_unique_factors = {}
		
		arr_n_1 = get_all_combinations_repeat(n=1, m=modulo).astype(np.int64)
		...
		for arr in arr_n:
			for d in arr_n_1:
				arr_next_idx = (np.sum(arr_n * arr, axis=1) + d) % modulo_big
				...
				l_t_factors_t_cycle_all.append((int(d), tuple(arr.tolist()), s_t_cycles))

		for d, t_arr, s_t_cycles in l_t_factors_t_cycle_all:
			t_factors = (d, t_arr)
		...
		d_modulo_to_l_t_factors_t_cycle_all[modulo] = l_t_factors_t_cycle_all
	...
	l_len_cycle = [t[1] for t in l_modulo_max_len_cycle]
	l_len_cycle_amount = [t[1] for t in l_modulo_max_len_cycle_amount]
	...
	print(f"n: {n}")
	print(f"- l_len_cycle: {l_len_cycle}")
	print(f"- l_len_cycle_amount: {l_len_cycle_amount}")
	...
	assert l_len_cycle == l_amount_max_len_cycle
