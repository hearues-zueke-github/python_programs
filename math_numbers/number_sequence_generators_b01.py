...
def get_increment_numbers_starting_zero(length, n):
...
def create_picture_2d_2_parameters_random_params(amount_args, modulo, max_power):
    ...
    home = os.path.expanduser("~")
    full_path = home+"/Pictures/sequences_pictures/parameters_2d_picture_args_{}_modulo_{}_max_power_{}".format(amount_args, modulo, max_power)
    if not os.path.exists(full_path):
        os.makedirs(full_path)
    os.chdir(full_path)
    ...
    # print("random x: {}".format(x))

    # x[fixed_idx] = fixed_values
    # print("x: {}".format(x))

    # print("Test the replaced function!")
    # print("f_stand(x, m): {}".format(f_stand(x, modulo)))
    # print("f_new({}, {}, {}): {}".format(x[params_idx[0]], x[params_idx[1]], modulo, f_new(x[params_idx[0]], x[params_idx[1]], modulo)))
    ...
    table_2d = np.vectorize(f_new)(x_vals, y_vals, m_vals)
    # print("table_2d:\n{}".format(table_2d))

    # table_2d_check = np.array([[f_new(x, y, modulo) for x in np.arange(0, modulo)] for y in np.arange(0, modulo)])
    # print("table_2d_check:\n{}".format(table_2d_check))
    ...
    # TODO: fill the pix field
    pix[:, :] = table_2d.reshape((modulo, modulo, 1))

    img = Image.fromarray(pix)
    img.save("param_idx_{}_{}.png".format(params_idx[0], params_idx[1]))
