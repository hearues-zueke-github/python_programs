...
inline bool isPrime(const uint64_t n, const vector<uint64_t>& primes) {
...
int main(int argc, char* argv[]) {
  ...
  std::ifstream fin("/tmp/primes_data_test_4.dat", std::ios_base::binary);
  ...
  const size_t l = found_values.size();
  for (size_t i = 0; i < l; ++i) {
    cout << found_values[i][3] << ", ";
  }
  cout << endl;
