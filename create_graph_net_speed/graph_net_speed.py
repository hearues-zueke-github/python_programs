...
if __name__ == '__main__':
    print('Hello World!')
    with open('saved_rx_bytes.txt', 'r') as f:
        lines = f.readlines()
    assert len(lines)==1
    line = lines[0]
    assert line[-1]==';'
    ...
    lst_rows = list(map(lambda x: (lambda y: (datetime.datetime.strptime(y[0], '%Y.%m.%d %H:%M:%S'), int(y[1])))(x.split(',')), lst_measurements))
    ...
    plt.plot(xs_sec, ys, 'b.', markersize=0.5)
    # plt.savefig('speedtest_average_all_values_{}.eps'.format(dt_str))
    plt.savefig('speedtest_average_all_values_{}.png'.format(dt_str), dpi=1000)
    ...
    plt.savefig('speedtest_average_min_median_max_r_{}_{}.png'.format(r, dt_str), dpi=1000)
