...
load_module_dynamically(**dict(var_glob=var_glob, name='utils_math_numbers', path=os.path.join(PYTHON_PROGRAMS_DIR, "math_numbers/utils_math_numbers.py")))
...
convert_n_to_other_base = utils_math_numbers.convert_n_to_other_base
convert_base_n_to_num = utils_math_numbers.convert_base_n_to_num
...
if __name__ == '__main__':
    ...
    i = 1
    while n > 0:
        l_b = convert_n_to_other_base(n=n, b=b_2)

        n = convert_base_n_to_num(l=(np.array(l_b) % b_1).tolist(), b=b_1)
        print(f"i: {i}, n: {n}")
