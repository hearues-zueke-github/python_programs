...
def calc_hashsum_from_file_path(file_path):
	...
	sha256sum =  h_sha256.hexdigest()
	sha3_256sum =  h_sha3_256.hexdigest()

	return sha256sum, sha3_256sum
...
if __name__ == '__main__':
	...
	l_dir_name_unfinished = [dir_path]
	l_l_dir_name = []
	step_i = 0

	l_data_dir_file = []

	d_dir_path_to_l_file_name = {}
	while l_dir_name_unfinished:
		step_i += 1
		print(f"step_i: {step_i}")

		l_dir_name_unfinished_next = []
		for dir_path in tqdm.tqdm(l_dir_name_unfinished):
			for root_dir, l_dir_name, l_file_name in os.walk(dir_path):
				break
			d_dir_path_to_l_file_name[dir_path] = l_file_name
			for file_name in l_file_name:
				l_data_dir_file.append((dir_path, file_name))

			for dir_name in l_dir_name:
				dir_path_2 = os.path.join(root_dir, dir_name)
				l_dir_name_unfinished_next.append(dir_path_2)

		if l_dir_name_unfinished_next:
			l_l_dir_name.append(l_dir_name_unfinished_next)

		l_dir_name_unfinished = l_dir_name_unfinished_next

	df = pd.DataFrame(data=l_data_dir_file, columns=['dir_path', 'file_name'], dtype=object)
	df['file_name_lower'] = pd.Series(data=[file_name.lower() for file_name in df['file_name'].values], index=df.index, dtype=object)
	...
	l_data_dir = []
	with alive_bar(len(l_dir_name), force_tty=True) as bar:
		for dir_name in l_dir_name:
			print(f"dir_name: {dir_name}")
			path = os.path.join(root_dir, dir_name)
			l_data_dir.append((dir_name, 'd', os.stat(path), os.path.islink(path)))
			bar()

	l_data_file = []
	with alive_bar(len(l_file_name)) as bar:
		for file_name in l_file_name:
			print(f"file_name: {file_name}")
			path = os.path.join(root_dir, file_name)
			sha256sum, sha3_256sum = calc_hashsum_from_file_path(file_path=file_name)
			l_data_file.append((file_name, 'f', os.stat(path), sha256sum, sha3_256sum, os.path.islink(path)))
			bar()

	df_dir = pd.DataFrame(data=l_data_dir, columns=['name', 'type', 'stat', 'is_link'])
	df_file = pd.DataFrame(data=l_data_file, columns=['name', 'type', 'stat', 'sha256sum', 'sha3_256sum', 'is_link'])
