...
int main(int argc, char* argv[]) {
	fstream f;
	f.open("/tmp/temp_file.cpp.txt", ios::out);

	const size_t amount = std::stoull(argv[1]);

	const auto time_1 = std::chrono::high_resolution_clock::now();
	RandomNumberDevice rnd = RandomNumberDevice(128, {0x00, 0x01, 0x02, 0x03, 0x04});
	// rnd.print_state();
	// rnd.print_values();
	...
	f << format("v_vec:{}\n", RandomNumberDevice::convert_vec_uint64_t_to_string(vec1));
